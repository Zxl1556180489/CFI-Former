...
class CollectiveDataset(data.Dataset):
    ...
    def __getitem__(self,index):  # index=591
        """
    ...
    def get_frames(self,frame):   # 传入get_frame（591） = frame（18,571）
        
        sid, src_fid = frame  # sid：18 src_fid：571
        
        if self.is_finetune:   # self.is_finetune = True for stage 1
            if self.is_training:  # self.is_training = True for stage 1  控制采样1帧还是10帧
                fid=random.randint(src_fid, src_fid+self.num_frames-1)  #在中心帧附近随机抽1帧 fid=578
                return [(sid, src_fid, fid)]
        
            else:
                return [(sid, src_fid, fid) 
                        for fid in range(src_fid, src_fid+self.num_frames)]  #在中心帧附近随机抽9帧
            
        else:  # self.is_training = False for stage 2
            if self.is_training:
                sample_frames=random.sample(range(src_fid,src_fid+self.num_frames),3)  # 训练的时候源代码稀疏采样3帧
                ...
                # sample_frames=[src_fid - 4, src_fid - 3, src_fid - 2, src_fid - 1, src_fid, src_fid + 1, src_fid + 2, src_fid + 3,
                #  src_fid + 4, src_fid + 5]  # 改变采样的样本帧 围绕中心中心帧前4 后5帧
                return [(sid, src_fid, fid) for fid in sample_frames]
    ...
    def load_samples_sequence(self,select_frames):
        ...
        for i, (sid, src_fid, fid) in enumerate(select_frames):  # select_frames = {list：1}[(18,571,578)]

            img = Image.open(self.images_path + '/seq%02d/frame%04d.jpg'%(sid,fid))  # (18,578) 打开随机采样的一帧

            img=transforms.functional.resize(img,self.image_size)  # resize为（720,480）
            img=np.array(img)
            ...
            temp_boxes=[]
            for box in self.anns[sid][src_fid]['bboxes']:   # 提取当前帧四个人的检测框位置
                y1,x1,y2,x2=box
            ...
            while len(temp_boxes)!=self.num_boxes:  # 由于跟踪器提供的box不一定是完整的(即一张图中boxes数目和设定的num_boxes不符)
